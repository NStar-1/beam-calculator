<script lang="ts">
  import {
    FixationEnum, type BeamEnd, type PointLoad, type AggregatedResult, isPhone, AnchorPoint
  ...
  export let firstPoint: BeamEnd = {isFixed: 1}
  export let lastPoint: BeamEnd = {isFixed: 0}
  export let loads: Array<PointLoad> = [{anchor: AnchorPoint.END, angle: 0, offset: 0, value: 10}]
  export let minMaxForce: [number, number] = [10, 10]
  export let length: number = 1000
  export let selectedLoad: number | null = null
  export let results: Array<AggregatedResult> = [{
    displacement: {x: 10, y: 0, z: 0, xx: 0, yy: 0, zz: 0},
    x: 1000,
    reaction: {x: 0, y: 0, z: 0, xx: 0, yy: 0, zz: 0},
  }]
  ...
  const fixationComponent: { [key in FixationEnum]?: any } = {
  ...
  $: fixationLeft = fixationComponent[firstPoint.isFixed];
  $: fixationRight = fixationComponent[lastPoint.isFixed];
  ...
  let loadsPos: Array<{x: number, y: number}>;
  ...
  $: loadsPos = loads.map((load, index) => {
    ...
    const result = results.find((d) => d.x === x);
    if (result) {
  ...
  <svg width="100%" height={totalHeight} shape-rendering="geometricPrecision">
    ...
    >
      <g class="drawing-local" transform="translate(0, {topOffset})">
        ...
        <path class="loaded-beam" d={curve.toString()} />
        {#each results as r}
